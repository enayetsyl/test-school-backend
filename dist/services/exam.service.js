...
export async function ensureEligibility(userId, step) {
    const user = await User.findById(userId).lean();
    if (!user)
        throw new AppError('UNAUTHORIZED', 'User not found', 401);
    if (step === 1) {
        if (user.isLockedFromStep1)
            throw new AppError('FORBIDDEN', 'Locked from retaking Step 1', 403);
        return;
    }
...
export async function answerQuestion(params) {
    ...
    if (session.status !== 'active')
        throw new AppError('FORBIDDEN', 'Session not active', 403);
    const now = new Date();
    ...
    if (!ans)
        throw new AppError('VALIDATION_ERROR', 'Question not in session', 400);
